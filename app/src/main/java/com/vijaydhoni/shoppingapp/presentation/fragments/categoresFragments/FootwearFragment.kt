...
import com.vijaydhoni.shoppingapp.databinding.FragmentFootwearBinding
...
@AndroidEntryPoint
class FootwearFragment : Fragment() {
    private lateinit var binding: FragmentFootwearBinding
    ...
    override fun onCreateView(
        ...
        showBottomNavView()
        binding = FragmentFootwearBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                categoryAdpter.loadStateFlow.collectLatest {
                    binding.footwearProgressBar.isVisible = it.append is LoadState.Loading
    ...
    private fun setUpvewAllProducts() {
        binding.productViewAll.setOnClickListener {
            val bundle = Bundle()
            bundle.putParcelable("category_name", Category("Footwear"))
            findNavController().navigate(
