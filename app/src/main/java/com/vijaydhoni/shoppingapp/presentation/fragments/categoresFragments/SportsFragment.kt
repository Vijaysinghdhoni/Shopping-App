...
import com.vijaydhoni.shoppingapp.databinding.FragmentSportsBinding
...
class SportsFragment : Fragment() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                categoryAdpter.loadStateFlow.collectLatest {
                    binding.sportsProgressBar.isVisible = it.append is LoadState.Loading
    ...
    private fun setUpvewAllProducts() {
        binding.productViewAll.setOnClickListener {
            val bundle = Bundle()
            bundle.putParcelable("category_name", Category("Sports"))
            findNavController().navigate(
